...
function onRejectFormSubmit(e) {
    ...
    fetch(`/study/applicant/reject?studyId=${studyForm.studyId}&memberId=${memberAnswers.memberId}&page=${pageNumber}`)
        .then(response => {
        ...
        .then(data => {
            if (data) {
                alert("승인 거부 되었습니다.");
                window.location.href = `/profile/table?page=${data.page}`;
            }
        })
...
function onFormSubmit(e) {
    ...
    fetch(`/study/applicant/approve?studyId=${studyForm.studyId}&memberId=${memberAnswers.memberId}&page=${pageNumber}`)
        .then(response => {
